...
void SeatTab::onRelease()
{
    int r = rowSpin->value(), c = colSpin->value();
    Seat* s = library->getSeat(r, c);
    if (!s || s->isFree()) { QMessageBox::information(this, QString::fromUtf8("提示"), QString::fromUtf8("该座位未被占用")); return; }
    ...
    for (int r = 0; r < 10; ++r) {
        for (int c = 0; c < 10; ++c) {
            ...
            if (s.status == SeatStatus::Occupied) {
                item->setBackground(QBrush(QColor(255, 150, 150))); // red = occupied
